...
/**
* COPY: 
* size of input sequence is minimum 4 (because that's what we need for an input + a copy)
* if we want to have more copies (k) then we need more cards (2k)
*/
#ifndef N
#define N 4
#endif
...
unsigned int isFinalState(struct state s) {
    /**
    * COPY:
    * this needs to be heavily altered
    * we need not just checks for a and b but also c and d
...
/**
* COPY:
* these aren't necessary for COPY, there we deal with isOne()
*/
/**
...
/**
 * Returns if the given sequence is a input sequence in the start state.
 */
unsigned int inputProbability(unsigned int start,
                              unsigned int arr[N]) {
    /**
    * COPY:
    * This has to be adapted for COPY, there is only isZero(arr) and isOne(arr)
    */
...
int main() {
    ...
    /**
    * COPY:
    * There are 2 start sequences for copy with isZero[start[0].arr] and isOne(start[1].arr)
    */
    ...
    for (unsigned int i = 2; i < N; i++) {
        assume (start[1].arr[i] == start[0].arr[i]);
    }
